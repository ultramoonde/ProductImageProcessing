...
def extract_product_info_with_consensus(image, tile_coords, image_name):
    ...
    try:
        ...
        for i, tile_info in enumerate(tile_coords, 1):
            ...
            try:
                # Convert tile to numpy array and prepare for consensus analysis
                import asyncio

                # Run consensus analysis on this tile
                # The method needs tile_image and text_region_image as numpy arrays
                # We'll use the same tile image for both parameters since we're analyzing the whole tile
                consensus_result = asyncio.run(analyzer.analyze_product_with_consensus(
                    tile_image=tile_image,
                    text_region_image=tile_image,  # Use same image for text region
                    analysis_mode="product"
                ))

                print(f'      ✅ Consensus result: {consensus_result.get("product_name", "Unknown")}')

                # Extract consensus data - the consensus system returns different field names
                desc = consensus_result.get("product_name", f"Product {i}")
                price = consensus_result.get("price", "0,00 €")
                brand = consensus_result.get("brand", "")
                weight = consensus_result.get("weight", "")
                quantity_raw = consensus_result.get("quantity", "")
                unit = consensus_result.get("unit", "")

                # Format quantity/weight properly
                quantity_str = f"{quantity_raw} {unit}".strip() if quantity_raw and unit else str(quantity_raw) if quantity_raw else ""
                ...
                # Calculate cost metrics using the same logic as before
                cost_per_kg, cost_per_piece = calculate_costs(desc, price)

                if cost_per_kg:
                    print(f'         💲 Cost per kg: {cost_per_kg}')
                if cost_per_piece:
